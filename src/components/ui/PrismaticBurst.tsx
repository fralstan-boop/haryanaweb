...
void main(){
    ...
    // OPIMIZATION: Dropped to 12 steps for buttery smooth 60fps performance!
    for (int i = 0; i < 12; ++i) {
        vec3 P = marchT * dir;
        P.z -= 2.0;
        float rad = length(P);
        ...
        // OPTIMIZATION: Multiply marchT by 0.75 (instead of 0.25) so it travels 3x faster through the color gradient space!
        // This ensures it reaches the blue and white colors even with only 12 math iterations.
        float saw = fract(marchT * 0.75);
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Optimization: Dropped drastically to 0.40 to ensure no scrolling resistance even on weaker GPUs.
    const baseDpr = window.devicePixelRatio || 1;
    const dpr = Math.min(baseDpr, 0.40); 
    ...
    container.addEventListener('pointermove', onPointer as any, { passive: true });
      ...
      io = new IntersectionObserver(
        entries => {
          if (entries[0]) {
            ...
            // If it just became visible, we need to restart the loop manually
            // to avoid JS main thread polling overhead when offscreen
            if (isVisibleRef.current && !wasVisible) {
                last = performance.now(); // reset delta so it doesn't jump
    ...
    const update = (now: number) => {
      // Safety guard: If React unmounted the component and disposed the mesh, abort immediately to prevent updateMatrixWorld crash.
      if (!meshRef.current) return;

      // If we aren't visible, cleanly kill the animation frame request to save 100% of CPU/GPU
